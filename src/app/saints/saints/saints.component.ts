...
export class SaintsComponent implements OnInit {
  ...
  ObservableCaballerosLista(){
    this.ListadoCaballeros = this.SaintsSrv.CaballerosListaOwn();
    if(this.ListadoCaballeros.length < 1){
      this.SaintsSrv.CaballerosLista()
        .subscribe(
          resultado=>(this.ListadoCaballeros=resultado,this.SaintsSrv.Add(resultado),this.bienvenida=`Haciendo Click sobre el ID puede ir a verificar el detalle...`,this.caballerosNotFound=true),
